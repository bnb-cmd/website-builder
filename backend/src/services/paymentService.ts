...
// Mock payment config for now - will be replaced with actual config
const paymentConfig = {
  stripe: {
  ...
  jazzcash: {
    ...
    returnUrl: process.env.JAZZCASH_RETURN_URL || 'https://example.com/return',
    cancelUrl: process.env.JAZZCASH_CANCEL_URL || 'https://example.com/cancel'
  },
  easypaisa: {
    merchantId: process.env.EASYPAISA_MERCHANT_ID || 'mock_merchant_id',
    password: process.env.EASYPAISA_PASSWORD || 'mock_password',
    returnUrl: process.env.EASYPAISA_RETURN_URL || 'https://example.com/return',
    cancelUrl: process.env.EASYPAISA_CANCEL_URL || 'https://example.com/cancel'
  }
}
...
export class PaymentService extends BaseService<any> {
  ...
  constructor() {
    ...
    if (paymentConfig.stripe.secretKey && !paymentConfig.stripe.secretKey.includes('mock')) {
  ...
  override async findById(id: string): Promise<any | null> {
    try {
      ...
      if (payment) {
        await this.setCached(cacheKey, payment, 1800) // 30 minutes
      }
  ...
  override async update(id: string, data: Partial<any>): Promise<any> {
    try {
      ...
      // Invalidate cache
      await this.invalidateCache(`payment:${id}`)
    ...
    try {
      if (!this.stripe) {
        ...
        await this.create({
          ...
          gatewayId: mockPaymentIntent.id,
          amount: new Decimal(data.amount),
          currency: data.currency,
          status: PaymentStatus.PENDING,
          gatewayData: mockPaymentIntent,
  ...
  private async confirmStripePayment(paymentIntentId: string, gatewayData?: any): Promise<boolean> {
    try {
      if (!this.stripe) {
        ...
        return gatewayData?.status === 'succeeded'
      }
