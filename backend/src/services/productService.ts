...
export class ProductService extends BaseService<Product> {
  ...
  override async create(data: Partial<Product>): Promise<Product> {
    try {
  ...
  override async update(id: string, data: Partial<Product>): Promise<Product> {
    try {
      this.validateId(id)
  ...
  // Custom product creation method
      ...
      const [
        ...
        this.prisma.product.count({
          where: {
            websiteId,
            trackInventory: true,
            inventory: { lte: 5 } // Low stock threshold
          }
        }),
  ...
  // Bulk Operations
  override async bulkCreate(products: Partial<Product>[]): Promise<Product[]> {
    try {
      const result = await this.prisma.product.createMany({
        data: products.map(product => ({
          ...
          comparePrice: product.comparePrice ? new Decimal(product.comparePrice) : null,
  ...
  override async bulkUpdate(updates: Array<{ id: string; data: Partial<Product> }>): Promise<Product[]> {
    try {
      const results: Product[] = []
      
      for (const update of updates) {
        const product = await this.updateProduct(update.id, update.data as any)
